...
def make_scene_script(script_name, min_v, max_v, min_h, max_h, max_t, repeat, show_input=True, frames_per_resize=10):
    ...
    if script_name == 'vertical_grow_shrink':
        ...
        stops = np.random.rand(10, 4) * np.array([max_v - min_v, max_h - min_h, 2 * max_t, 2 * max_t]
        ...
        shift_l = np.concatenate(
            [l(stop_0[0], stop_1[0], frames_per_resize) for stop_0, stop_1 in zip(np.vstack(([0, 0], stops)), stops)]
        )

        shift_r = np.concatenate(
            [l(stop_0[1], stop_1[1], frames_per_resize) for stop_0, stop_1 in zip(np.vstack(([0, 0], stops)), stops)]
        )

    return [[-1, -1, -1, -1]] * 20 + list(zip(size_v, size_h, shift_l, shift_r)) * repeat if show_input else list(
        zip(size_v, size_h, shift_l, shift_r)
    ) * repeat
